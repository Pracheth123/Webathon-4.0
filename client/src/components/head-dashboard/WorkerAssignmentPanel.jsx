...
export default function WorkerAssignmentPanel({
  issue,
  assignedTeam,
  verified,
  onAssign,
  ...
  return (
    ...
    >
      <span className="text-meta text-primary block mb-2">
        WORKER ASSIGNMENT
      </span>
      <h3 className="text-sm font-bold text-base-content mb-3 line-clamp-2">
        {issue.title}
      </h3>
      ...
      <div className="glass rounded-xl p-3 mb-3">
        <div className="flex items-center justify-between text-xs text-base-content/60">
          ...
          <span className="text-base-content">
            {assignedTeam || issue.assignedTo || "Not assigned"}
          </span>
        </div>
      </div>
      ...
      {!assignedTeam && !issue.assignedTo && (
        <div className="mb-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {["Ward Crew A", "Ward Crew B", "Rapid Response Unit"].map(
              (team) => (
                <button
                  key={team}
