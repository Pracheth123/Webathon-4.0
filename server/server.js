...
const startServer = async () => {
  try {
    ...
    // Try to bind to a port; if taken, try the next one up to a limit
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        // graceful handling of unhandled rejections while server is up
    ...
    console.error(`Failed to bind server after ${maxAttempts} attempts starting at port ${DEFAULT_PORT}.`);
    process.exit(1);
  } catch (error) {
